### normality-bayes-inference.R ###################################################################
# purpose: library rstan:
...
for (instance in seq(1, number.simulation)) {
    ...
    invisible({capture.output({
        stan.fit <- stan(
            file = paste0(stan.dir, 'normality-bayes-inference.stan'),
            data = data,
            chains = 6,
            iter = 1000,
            cores = 6,
    ...
    cat(problematic.coef, 'coefs poorly estimated with rhat < 1.1 \n');
...
saveRDS(fit.coef, paste0(result.dir, 'normality-assumption-coefs.rds'));
